...
def get_data_model(args):
    ...
    DATA_PATH = {
        "ner": ["./jc-data/ner.json", "utils/ner_dataset.py", args.max_num_instances_ner],
    }
...
def evaluate(args):
    ...
    def compute_ie_metrics(dataset, inputs, preds, save_prefix=None):
        ...
        if save_prefix is not None:
            with open(os.path.join(checkpoint_path, f"{save_prefix}_{args.use_all_templates}_ner_predictions.jsonl"), "w") as fout:
            ...
            wandb.save(os.path.join(checkpoint_path, f"{save_prefix}_{args.use_all_templates}_ner_predictions.jsonl"))
        ...
        ner_f1_result_per_template = {x: y for x, y in result_per_category_template.items() if "ent-F1" in x}
        result.update(compute_mean_std(ner_f1_result_per_template, "ent-F1"))

        prediction_lens = [np.count_nonzero(pred != tokenizer.pad_token_id) for pred in preds]
        result["gen_len"] = np.mean(prediction_lens)
        result = {k: round(v, 4) for k, v in result.items()}
        with open(os.path.join(checkpoint_path, f"{save_prefix}_{args.use_all_templates}_ner_results.json"), "w") as fout:
            json.dump(result, fout, indent=4)
        wandb.save(os.path.join(checkpoint_path, f"{save_prefix}_{args.use_all_templates}_ner_results.json"))
        return result
...
if __name__ == "__main__":
    ...
    parser.add_argument('--max_num_instances_ner', default=500, type=int)
