...
def get_data_model(args):
    ...
    DATA_PATH = {
        "ee": ["./jc-data/ee.json", "utils/ee_dataset.py", args.max_num_instances_ee],
    }
...
def remove_duplicates(events):
    ...
    for event in new_events:
        if type(event) is dict and "arguments" in event.keys():
            new_arguments = []
...
def evaluate(args):
    ...
    def compute_ie_metrics(dataset, inputs, preds, save_prefix=None):
        ...
        if save_prefix is not None:
            with open(os.path.join(checkpoint_path, f"{save_prefix}_{args.use_all_templates}_ee_predictions.jsonl"), "w") as fout:
            ...
            wandb.save(os.path.join(checkpoint_path, f"{save_prefix}_{args.use_all_templates}_ee_predictions.jsonl"))
        ...
        ee_trigger_result_per_template = {x: y for x, y in result_per_category_template.items() if "evt-trigger-F1" in x}
        ee_role_result_per_template = {x: y for x, y in result_per_category_template.items() if "evt-role-F1" in x}

        result.update(compute_mean_std(ee_trigger_result_per_template, "evt-trigger-F1"))
        result.update(compute_mean_std(ee_role_result_per_template, "evt-role-F1"))

        prediction_lens = [np.count_nonzero(pred != tokenizer.pad_token_id) for pred in preds]
        result["gen_len"] = np.mean(prediction_lens)
        result = {k: round(v, 4) for k, v in result.items()}
        with open(os.path.join(checkpoint_path, f"{save_prefix}_{args.use_all_templates}_ee_results.json"), "w") as fout:
            json.dump(result, fout, indent=4)
        wandb.save(os.path.join(checkpoint_path, f"{save_prefix}_{args.use_all_templates}_ee_results.json"))
...
if __name__ == "__main__":
    ...
    parser.add_argument('--max_num_instances_ee', default=1000, type=int)
